(() => {
  ...
  function mount(root, config = {}) {
    const appRoot = typeof root === 'string' ? document.querySelector(root) : root;
    if (!(appRoot instanceof Element)) throw new TypeError('KuechenStilfinder.mount requires a root element.');
    if (mounted.has(appRoot)) return mounted.get(appRoot);

    const settings = Object.freeze({
      submitUrl: String(config.submitUrl || 'api/submit.php'),
      progressSaveUrl: String(config.progressSaveUrl || 'api/stilfinder_progress.php'),
      csrfToken: String(config.csrfToken || ''),
      assetBase: new URL(String(config.assetBase || './'), document.baseURI).href,
  ...
  state.source = /^[A-Za-z0-9._:-]{1,80}$/.test(String(settings.source || '')) ? String(settings.source) : 'kuechen-kompass';
  state.campaign = normalizeCampaign(settings.campaign || campaignFromLocation());
  if (settings.initialState) applyInitialState(settings.initialState);

  function applyInitialState(raw) {
    const validQuestionIds = new Set(questions.map(q => q.id));
    const answers = {};
    if (raw.answers && typeof raw.answers === 'object') {
      Object.entries(raw.answers).forEach(([id, values]) => {
        if (!validQuestionIds.has(id) || !Array.isArray(values)) return;
        answers[id] = values.filter(v => typeof v === 'string').slice(0, 20);
      });
    }
    const details = raw.details && typeof raw.details === 'object' ? raw.details : {};
    const skipped = Array.isArray(raw.skipped) ? raw.skipped.filter(id => validQuestionIds.has(id)) : [];
    const resumeIndex = questions.findIndex(q => q.id === raw.current_step);
  ...
  const selectionMessage = byId('selectionMessage');
  let autoAdvanceTimer = null;
  const reportedEmbedEvents = new Set();
  const csrfField = byId('leadForm')?.elements?.csrf_token;
  if (csrfField && settings.csrfToken) csrfField.value = settings.csrfToken;
  window.addEventListener('kuechen-kompass:set-context', event => {
    const visitorId = String(event.detail?.visitor_id || '').trim();
  ...
  function bind() {
    window.KuechenKompassImages?.bindElement?.(appRoot.querySelector('.intro-visual img'));
    byId('startButton').addEventListener('click', start);
    byId('restartButton').addEventListener('click', restart);
    byId('backButtonTop').addEventListener('click', back);
  ...
  function campaignFromLocation() {
    ...
    return normalizeCampaign(Object.fromEntries(
  ...
  function reportEmbedEvent(name, properties = {}, dedupeKey = name) {
    const allowed = new Set(['view', 'start', 'style_result', 'project_questions_start', 'contact_open', 'lead', 'step', 'progress_save']);
    if (!allowed.has(name) || reportedEmbedEvents.has(dedupeKey)) return;
    reportedEmbedEvents.add(dedupeKey);
  ...
  function assetUrl(path) {
    return new URL(String(path || ''), settings.assetBase).href;
  }
  ...
  function renderQuestion(focusOptionId = '') {
    ...
    byId('questionCounter').textContent = `${state.current + 1} von ${questions.length}`;
    byId('questionTitle').textContent = q.title;
    const multipleHint = byId('multipleHint');
    ...
    byId('questionHelp').textContent = q.help || '';
    ...
    byId('backButtonTop').disabled = state.current === 0;
    byId('skipButton').hidden = Boolean(q.required);
    updateNext(q);
    renderJourney();
    if (focusOptionId) {
  ...
  function revealResult(result) {
    ...
    const resultImageUrl = assetUrl(styleCopy[result.primary].image);
  ...
  async function submitResultGate(event) {
    ...
    const error = byId('resultGateError');
    error.textContent = '';
    ...
    try {
      const response = await fetch(settings.progressSaveUrl, {
        ...
        body: JSON.stringify({
          ...
          current_step: questions[state.current]?.id || '',
  ...
  function optionImage(questionId, optionId) {
    ...
    return path ? assetUrl(path) : generatedOptionImage(questionId, optionId);
  }

  function renderBars(result) {
    const top = result.ranked.slice(0, 3);
    byId('styleBars').innerHTML = top.map(item => {
  ...
  function renderStyleGuidance(result) {
    ...
    const list = byId('resultPrinciples');
  ...
  function renderMoodboard() {
    ...
    byId('moodboard').innerHTML = (options.length ? options : [option('base', 'Ihre Auswahl', '', '#c9b79f', {})])
  ...
  function renderPlanningSummary() {
    ...
    const wrapper = byId('planningSummary');
    const grid = byId('planningGrid');
    if (!rows.length) {
  ...
  function openContact() {
    ...
    if (state.gateEmail && !form.elements.email.value) form.elements.email.value = state.gateEmail;
    byId('contactSummary').innerHTML = `<strong>${escapeHtml(result.title)}</strong><p>${completion()} % der Planungsfragen sind bereits beantwortet.</p>`;
    renderJourney('result');
    showView('contact');
  ...
  function escapeHtml(value) {
    ...
    const instance = Object.freeze({ restart });
    mounted.set(appRoot, instance);
    return instance;
  }

  window.KuechenStilfinder = Object.freeze({ mount });
})();